# Java AI RAG Agent Platform

[中文说明](README_CN.md)

[Project questions and answers (Chinese)](QUESTIONS_CN.md)

A Java AI application engineering portfolio project built incrementally with Java and Spring Boot.

## Current Iteration

Iteration 3.5 adds short-term conversation context to the DeepSeek chat integration:

- Java 17 and Maven
- Spring Boot 3.5
- Spring AI 1.1
- DeepSeek OpenAI-compatible chat API
- Synchronous and SSE streaming chat endpoints
- Optional `conversationId` for multi-turn context
- In-memory conversation window with configurable max messages
- Conversation create, list, inspect, and clear endpoints
- Explicit `chunk`, `done`, and `error` stream events
- Configurable system prompt for company instructions
- Health check, request validation, and consistent API errors
- Automated tests that never call the real model API

Conversation memory is in-memory only and is lost after application restart.
Document processing, RAG, Tool Calling, and MCP are not implemented yet.

## Requirements

- Java 17 or later
- Maven 3.9 or later
- A valid DeepSeek API key

## DeepSeek Configuration

The application reads model settings from environment variables. Never commit a real API key.

| Environment variable | Required | Default | Description |
| --- | --- | --- | --- |
| `AI_PROVIDER` | No | `deepseek` | Logical model provider name |
| `DEEPSEEK_API_KEY` | Yes | none | DeepSeek API credential |
| `DEEPSEEK_BASE_URL` | No | `https://api.deepseek.com` | DeepSeek API base URL |
| `DEEPSEEK_MODEL` | No | `deepseek-v4-flash` | Chat model name |
| `AI_SYSTEM_PROMPT` | No | general assistant prompt | Instructions applied to every chat |
| `AI_MEMORY_MAX_MESSAGES` | No | `20` | Maximum messages retained per conversation |
| `SERVER_PORT` | No | `8080` | Local HTTP port |
| `CHAT_STREAM_TIMEOUT` | No | `5m` | Maximum asynchronous request duration |

PowerShell example:

```powershell
$env:DEEPSEEK_API_KEY = "your-new-api-key"
$env:AI_SYSTEM_PROMPT = "You are a professional company assistant."
mvn spring-boot:run
```

For IntelliJ IDEA, open **Run/Debug Configurations**, select `AiRagAgentPlatformApplication`, and add `DEEPSEEK_API_KEY` under **Environment variables**.

The `.env.example` file is only a reference. Spring Boot does not load `.env` automatically.

## Run Locally

Run tests without making any real model request:

```bash
mvn clean test
```

Start the application after configuring the API key:

```bash
mvn spring-boot:run
```

## API Endpoints

### Health Check

```http
GET /api/health
```

### Chat

```http
POST /api/chat
Content-Type: application/json

{
  "message": "Explain dependency injection in Java."
}
```

Response:

```json
{
  "answer": "The answer generated by DeepSeek",
  "conversationId": null
}
```

Blank messages return HTTP `400`. Model provider failures return HTTP `502` with the error code `MODEL_PROVIDER_ERROR`.

To connect context, reuse a `conversationId`:

```http
POST /api/chat
Content-Type: application/json

{
  "message": "My name is Alex.",
  "conversationId": "demo-1"
}
```

Then send another request with the same id:

```http
POST /api/chat
Content-Type: application/json

{
  "message": "What is my name?",
  "conversationId": "demo-1"
}
```

### Streaming Chat

```http
POST /api/chat/stream
Accept: text/event-stream
Content-Type: application/json

{
  "message": "Explain Spring AI in three sentences.",
  "conversationId": "demo-1"
}
```

Test without client-side buffering:

```powershell
$body = @{ message = "Explain Spring AI in three sentences."; conversationId = "demo-1" } | ConvertTo-Json -Compress
$body | curl.exe -N -X POST http://localhost:8080/api/chat/stream `
  -H "Accept: text/event-stream" `
  -H "Content-Type: application/json" `
  --data-binary '@-'
```

The stream emits `chunk` events, followed by one `done` event. A provider failure after streaming begins is emitted as an `error` event because the HTTP response has already started.

### Conversations

Create a new conversation id:

```http
POST /api/conversations
```

List known in-memory conversations:

```http
GET /api/conversations
```

Inspect the retained memory window:

```http
GET /api/conversations/demo-1
```

Clear a conversation:

```http
DELETE /api/conversations/demo-1
```

## Project Structure

```text
src/main/java/com/example/airagagentplatform
|-- config
|   |-- AiChatConfiguration.java  # Creates the Spring AI ChatClient
|   `-- AiChatProperties.java     # Application-level AI settings
|-- controller                    # REST endpoints and API errors
|-- domain                        # Core domain models added later
|-- dto
|   |-- ChatRequest.java          # Chat request with optional conversationId
|   |-- ChatStreamResponse.java   # SSE event data
|   `-- Conversation*.java        # Conversation management DTOs
|-- repository                    # Persistence components added later
`-- service
    |-- ChatService.java          # Provider-independent chat contract
    |-- ConversationService.java  # Conversation memory contract
    |-- DeepSeekChatService.java  # DeepSeek implementation
    `-- InMemoryConversationService.java
```

## Delivery Sequence

1. Spring Boot foundation and local chat endpoint - complete
2. DeepSeek model API integration - complete
3. Streaming output - complete
3.5. Conversation context and memory - complete
4. Document upload
5. Text chunking
6. Embedding generation
7. Vector retrieval
8. RAG answers
9. Tool Calling
10. MCP integration
